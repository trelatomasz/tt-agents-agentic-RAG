...
def error(request_id: str, code: str, message: str, retryable: bool, fallback: str, status: int):
    body = ErrorResponse(request_id=request_id, error=ErrorBody(code=code, message=message, retryable=retryable, fallback=fallback))
...
async def ask(payload: AskRequest):
    try:
        return await service.ask(payload.query, payload.request_id, settings.request_timeout_seconds)
...
async def stream(payload: AskRequest, request: Request):
    async def events() -> AsyncIterator[str]:
        ...
        task = asyncio.create_task(service.ask(payload.query, payload.request_id, settings.request_timeout_seconds))
        while not task.done():
